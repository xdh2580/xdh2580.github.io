var store = [{
        ...
        "title": "Postman相关",
        "excerpt":"Postman API开发与测试 完整学习大纲 一、 Postman 概述与核心价值 1.1 Postman是什么？ 定义：一个功能强大的API协作平台，用于设计、开发、测试、记录和监控API。 从API客户端到API平台：单体应用、工作区、API网络、公网。 1.2 核心功能模块 API客户端：发送HTTP请求。 API设计：遵循OpenAPI/Swagger规范。 自动化测试：编写测试脚本。 Mock服务器：模拟API响应。 监控：计划任务与API运行状况检查。 文档：自动生成与发布API文档。 协作：团队工作区、版本控制、权限管理。 1.3 核心概念 工作区 (Workspace)：个人、团队、私人和公共工作空间。 集合 (Collection)：请求和相关元素的分组容器。 环境 (Environment)：包含变量的键值对集合，用于不同上下文（开发、测试、生产）。 变量 (Variables)：在不同作用域（全局、集合、环境、局部、数据）中存储和重用值。 二、 入门与环境搭建 2.1 获取与安装Postman 桌面应用（推荐） vs Web版。 下载与安装。 注册Postman账户（启用同步与协作功能）。 2.2 用户界面导览 侧边栏：历史记录、集合、API、环境、Mock服务器、监控器、Flows。 主工作区：请求构建器、响应查看器。 顶部栏：新建、导入、Runner、启动、协作、同步。 2.3 创建第一个请求 新建请求选项卡。 选择HTTP方法：GET, POST, PUT, DELETE,...","categories": ["Postman"],
        "tags": ["Postman"],
        "url": "/postman/Postman%E7%9B%B8%E5%85%B3/",
        "teaser": null
      },{
        "title": "Jmeter",
        "excerpt":"Apache JMeter 性能测试 完整学习大纲 一、 JMeter 概述与性能测试基础 1.1 JMeter是什么？ 定义：Apache JMeter™是一个纯Java桌面应用程序，用于功能行为和性能测试。 主要用途：负载测试、压力测试、功能测试、回归测试、数据库测试、Web服务测试。 特性：开源、跨平台、支持多协议、可扩展。 1.2 性能测试基本概念 并发用户 vs 吞吐量 vs 响应时间。 负载测试：验证系统在预期负载下的行为。 压力测试：找到系统在极限负载下的瓶颈和崩溃点。 疲劳测试：长时间、高稳定性的负载测试。 容量规划测试：确定系统在满足性能指标下的最大容量。 关键性能指标（KPI）： 响应时间（平均、P90/P95/P99）。 吞吐量（Requests/Second, Transactions/Second）。 错误率。 资源利用率（CPU、内存、磁盘I/O、网络）。 1.3 JMeter 架构与核心组件 工作原理：基于线程组的模拟并发。 测试计划：JMeter脚本的根节点。 线程组：模拟并发用户的基础单元。 取样器：发送请求的核心组件（HTTP, JDBC, FTP等）。 逻辑控制器：控制取样器的执行顺序和逻辑。 监听器：收集、查看和分析测试结果。 配置元件：为取样器提供配置信息（参数、头、Cookie等）。 前置/后置处理器：在请求发送前/后处理数据。 断言：验证响应结果。 定时器：控制请求的发送节奏。 二、 环境搭建与第一个测试计划 2.1 系统要求与安装 Java环境：安装JRE或JDK（推荐JDK...","categories": ["Jmeter"],
        "tags": ["Jmeter"],
        "url": "/jmeter/Jmeter/",
        "teaser": null
      },{
        "title": "SQL相关",
        "excerpt":"SQL 系统学习与知识体系大纲 一、 引言与概述 1.1 SQL是什么？ 定义：结构化查询语言 用途：用于管理关系数据库系统（RDBMS） 标准：ANSI SQL 与各厂商扩展（T-SQL, PL/SQL等） 1.2 数据库基本概念 数据库 (Database) 表 (Table) 行/记录 (Row/Record) 列/字段 (Column/Field) 主键 (Primary Key) 与外键 (Foreign Key) 索引 (Index) 二、 环境搭建与初步实践 2.1 常见数据库管理系统 (DBMS) 简介 MySQL / MariaDB PostgreSQL SQLite Microsoft SQL Server Oracle Database 2.2 选择与安装一个DBMS (以MySQL/PostgreSQL为例) 2.3...","categories": ["SQL Mysql"],
        "tags": ["SQL","Mysql"],
        "url": "/sql%20mysql/SQL%E7%9B%B8%E5%85%B3/",
        "teaser": null
      },{
        "title": "Selenium相关",
        "excerpt":"Selenium Web自动化测试与爬虫 系统学习大纲 一、 Selenium 概述与生态 1.1 Selenium是什么？ 定义：一套用于Web应用程序自动化测试和网页交互的工具集。 核心组件：Selenium IDE, Selenium WebDriver, Selenium Grid。 与Puppeteer、Playwright、Cypress等工具的简要对比。 1.2 核心组件详解 Selenium IDE：浏览器插件，用于录制与回放脚本（主要用于原型和简单脚本生成）。 Selenium WebDriver：核心，提供了一套与浏览器进行原生交互的API（通过浏览器驱动程序）。 Selenium Grid：用于分布式测试，支持在多台机器、多种浏览器/操作系统上并行执行测试。 1.3 工作原理 WebDriver API -&gt; 浏览器驱动 (如 chromedriver, geckodriver) -&gt; 真实浏览器。 JSON Wire Protocol (旧) / W3C WebDriver Protocol (新)。 二、 环境搭建与第一个脚本 2.1 语言与工具选择 支持的编程语言：Python (主流)、Java、C#、JavaScript、Ruby。...","categories": ["Selenium"],
        "tags": ["Selenium"],
        "url": "/selenium/Selenium%E7%9B%B8%E5%85%B3/",
        "teaser": null
      }]
