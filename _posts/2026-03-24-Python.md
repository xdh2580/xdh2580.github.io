...
*   **4.5 字符串操作**
    ...
    **一、字符串基础**
    ...
    3. 原始字符串
        ...
        ```python
        # 原始字符串（不转义）
        path1 = "C:\\Users\\Alice\\Documents"  # 需要转义
        path2 = r"C:\Users\Alice\Documents"    # 原始字符串，不需要转义

        # 正则表达式常用
        regex1 = "\\d+\\.\\d+"  # 匹配数字
        regex2 = r"\d+\.\d+"    # 更清晰的原始字符串
        ```
    ...
    **三、字符串格式化**
    ...
    1. 三种格式化方式对比
        ...
        msg3 = "Name: {0}, Age: {1}".format(name, age)
        msg4 = "Name: {n}, Age: {a}".format(n=name, a=age)

        # 3. f-string (Python 3.6+)
        msg5 = f"Name: {name}, Age: {age}"
        msg6 = f"Next year: {age + 1}"
        msg7 = f"Name: {name.upper()}"
        ```
    ...
    * 编码基础
        ...
        # 解码为字符串
        text1 = utf8_bytes.decode("utf-8")   # 你好，世界
        text2 = gbk_bytes.decode("gbk")      # 你好，世界

        # 常见编码
        encodings = ["utf-8", "gbk", "gb2312", "ascii", "latin-1", "utf-16"]
        ```
    ...
    * 处理编码问题
        ...
        # 错误处理方式
        text.encode("utf-8").decode("ascii", errors="ignore")   # 忽略错误字符
        text.encode("utf-8").decode("ascii", errors="replace")  # 用?替换
        text.encode("utf-8").decode("ascii", errors="backslashreplace")  # 用\xhh替换

        # 检测编码
        import chardet
        data = "你好".encode("gbk")
        result = chardet.detect(data)  # {'encoding': 'GB2312', 'confidence': 0.99}
        ```
    ...
    * 内置常量
        ...
        print(string.whitespace)        # 空白字符: 空格、\t、\n、\r、\v、\f
    ...
    **六、字符串方法详解**
    ...
    1. 判断方法
        ...
        print(s.istitle())     # True (每个单词首字母大写)
    ...
    * 替换与分割
        ...
        # 替换
        print(text.replace(",", ";"))  # apple;banana;orange;grape
        print(text.replace(",", ";", 2))  # apple;banana;orange,grape
    ...
    **八、字符串模板**
    ...
    print(result)  # Hello, Alice! Today is Monday.

    # 2. 安全字典替换
    data = {"name": "Bob", "day": "Tuesday"}
    result2 = template.substitute(data)
    print(result2)  # Hello, Bob! Today is Tuesday.

    # 3. 安全替换（缺失键时不报错）
    result3 = template.safe_substitute(name="Charlie")
    print(result3)  # Hello, Charlie! Today is $day.

    # 4. 自定义分隔符
    class MyTemplate(Template):
        delimiter = "#"  # 使用#作为分隔符

    template2 = MyTemplate("Hello, #name!")
    print(template2.substitute(name="David"))  # Hello, David!
    ```
    ...
    * 字符串连接效率
        ...
        print(f"+ 连接耗时: {time_plus:.4f}秒")
        print(f"join连接耗时: {time_join:.4f}秒")

        # 4. 格式化性能比较
        name, age = "Alice", 25
        time_format = timeit.timeit(lambda: "Name: {}, Age: {}".format(name, age), number=100000)
        time_fstring = timeit.timeit(lambda: f"Name: {name}, Age: {age}", number=100000)

        print(f"format耗时: {time_format:.4f}秒")
        print(f"f-string耗时: {time_fstring:.4f}秒")
        ```
    ...
    **十、高级字符串处理**
    ...
    1. 字符串与字节
        ...
        print(byte_data)        # b'Python\xe7\xbc\x96\xe7\xa8\x8b'

        # bytes 和 bytearray
        b = b"hello"  # 不可变字节
        ba = bytearray(b"hello")  # 可变字节数组
        ba[0] = 72  # 修改第一个字节
    ...
    2. 正则表达式
        ...
        items = re.split(r'[,\s;]+', text2)
        print(items)  # ['apple', 'banana', 'orange', 'grape']

        # 匹配对象
        match = re.search(r'(\w+)@([\w.]+)', text)
        if match:
            print(f"完整匹配: {match.group(0)}")  # alice@example.com
            print(f"用户名: {match.group(1)}")    # alice
    ...
    * 常用模式
    ...
    s = "Python"
    reverse1 = s[::-1]  # 切片法
    reverse2 = ''.join(reversed(s))  # 内置函数
    print(reverse1, reverse2)  # nohtyP nohtyP
    ...
    # 4. 提取数字
    import re
    text = "Price: $123.45, Weight: 12.5kg"
    numbers = re.findall(r'\d+\.?\d*', text)
    print(numbers)  # ['123.45', '12.5']
    ...
    * 字符处理
    ...
    print(words)  # ['programming', 'powerful', 'language', 'Python', 'is', 'a']

    # 4. 生成随机字符串
    import random
    import string
    random_str = ''.join(random.choices(string.ascii_letters + string.digits, k=10))
    print(random_str)  # 类似: 'X5j8K9mN2p'
    ```
    ...
    **关于各数据结构中查找元素的方法及对比**  
    ...
    | 数据结构 | 是否有序 | 是否可变 | 主要查找方法 | 时间复杂度 |
    |---------|---------|---------|-------------|-----------|
    | 列表(list) | 有序 | 可变 | `index()`, `in`, 切片 | O(n) |
    | 元组(tuple) | 有序 | 不可变 | `index()`, `in`, 切片 | O(n) |
    | 集合(set) | 无序 | 可变 | `in` | O(1)平均 |
    | 字典(dict) | 有序(3.7+) | 可变 | `in`(键), `get()`, 索引 | O(1)平均 |
    | 字符串(str) | 有序 | 不可变 | `find()`, `index()`, `in` | O(n) |

    | 特性 | find() | index() |
    |------|--------|---------|
    | **返回结果** | 找到返回索引，否则-1 | **找到返回索引，否则抛出异常** |
    | **适用类型** | 字符串 | 字符串、列表、元组 |
    | **异常处理** | 不需要 | 需要try-except处理 |
    | **右向查找** | 有rfind() | 有rindex() |
    | **典型用法** | 检查是否存在并获取位置 | 确信元素存在时使用 |
...
**python函数进阶：**[pyhon函数进阶]({%post_url /Details/2026-04-05-python函数%})

**python常用内置函数：**[pyhon常用内置函数]({%post_url /Details/2026-04-16-Python常用内置函数%})
